...
const CookieBanner = () => {
  ...
  const loadGA = () => {
    ...
    gtag('js', new Date());
    gtag('config', 'G-7ZGKELGBFJ', { 'anonymize_ip': true });
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          animate={{ y: 0 }}
          exit={{ y: 100 }}
          className="fixed bottom-0 left-0 right-0 bg-gray-100 border-t border-gray-200 z-[9999] py-3 px-4 shadow-2xl"
        >
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <button
                ...
                className="px-4 py-1.5 text-xs font-bold bg-gray-900 text-white rounded hover:bg-black transition-colors"
              >
                Accepteren
              </button>
              <Link
                to="/privacy"
                className="px-4 py-1.5 text-xs font-medium text-blue-600 hover:underline"
              >
                Privacy
              </Link>
            </div>
