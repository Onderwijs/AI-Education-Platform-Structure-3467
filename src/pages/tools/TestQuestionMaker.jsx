...
Instructies:
1. Maak de vragen concreet en passend bij het niveau.
2. Lever eerst alleen de vragen, duidelijk genummerd.
3. Geef daarna in een apart blok de modelantwoorden (correctiemodel).`;
            ...
            <form onSubmit={generateQuiz} className="space-y-5">
              ...
              <div>
                ...
                <input 
                  ...
                  placeholder="Bijv. Klimaatverandering" 
              ...
              <div>
                ...
                <div className="space-y-2 bg-gray-50 p-3 rounded-lg border border-gray-200">
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" name="multipleChoice" checked={formData.types.multipleChoice} onChange={handleTypeChange} className="rounded text-green-600 focus:ring-green-500" />
                    <span className="text-sm text-gray-700">Meerkeuze</span>
                  </label>
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" name="open" checked={formData.types.open} onChange={handleTypeChange} className="rounded text-green-600 focus:ring-green-500" />
                    <span className="text-sm text-gray-700">Open vragen</span>
                  </label>
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" name="trueFalse" checked={formData.types.trueFalse} onChange={handleTypeChange} className="rounded text-green-600 focus:ring-green-500" />
                    <span className="text-sm text-gray-700">Stellingen (Juist/Onjuist)</span>
                  </label>
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" name="fillIn" checked={formData.types.fillIn} onChange={handleTypeChange} className="rounded text-green-600 focus:ring-green-500" />
                    <span className="text-sm text-gray-700">Invulvragen / Gatentekst</span>
                  </label>
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Moeilijkheid (Taxonomie)</label>
                <select 
                  name="difficulty" value={formData.difficulty} onChange={handleChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent bg-white"
                >
                  <option value="Onthouden">Vooral onthouden (Kennis)</option>
                  <option value="Begrijpen">Begrijpen (Inzicht)</option>
                  <option value="Toepassen">Toepassen</option>
                  <option value="Analyseren">Analyseren & Evalueren</option>
                </select>
              </div>

              {/* Bron Keuze */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Bronmateriaal</label>
                <div className="flex space-x-4 mb-3">
                  <label className={`flex-1 flex items-center justify-center space-x-2 p-2 rounded-lg border cursor-pointer transition-colors ${formData.sourceType === 'text' ? 'bg-green-50 border-green-500 text-green-700' : 'border-gray-300 hover:bg-gray-50'}`}>
                    <input type="radio" name="sourceType" value="text" checked={formData.sourceType === 'text'} onChange={handleChange} className="hidden" />
                    <SafeIcon icon={FiType} />
                    <span className="text-sm font-medium">Eigen tekst</span>
                  </label>
                  <label className={`flex-1 flex items-center justify-center space-x-2 p-2 rounded-lg border cursor-pointer transition-colors ${formData.sourceType === 'upload' ? 'bg-green-50 border-green-500 text-green-700' : 'border-gray-300 hover:bg-gray-50'}`}>
                    <input type="radio" name="sourceType" value="upload" checked={formData.sourceType === 'upload'} onChange={handleChange} className="hidden" />
                ...
                {formData.sourceType === 'text' && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>
                    <textarea 
                      name="sourceText" 
                      rows="4" 
                      placeholder="Plak hier de tekst waarover de vragen moeten gaan..." 
                ...
                {formData.sourceType === 'upload' && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-xs text-gray-500 bg-blue-50 p-3 rounded border border-blue-100">
                    ℹ️ Je uploadt het bestand straks zelf direct in ChatGPT of Gemini. Wij sturen geen bestanden door.
                  </motion.div>
                )}
              </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            {!result ? (
              ...
              <motion.div 
                ...
                className="space-y-6"
              >
                {/* Result Block 1: Structure */}
                <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                  <div className="bg-green-50 p-4 border-b border-green-100 flex justify-between items-center">
                    <h3 className="font-bold text-green-900 flex items-center gap-2">
                      <SafeIcon icon={FiList} /> Voorstel Toetsstructuur
                    </h3>
                  </div>
                  <div className="p-6">
                    <div className="mb-4 text-sm text-gray-600">
                      <p><strong>Titel:</strong> Toets {formData.subject} – {formData.topic}</p>
                      <p><strong>Focus:</strong> {formData.difficulty}</p>
                    </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100 text-xs text-gray-500 italic">
                      Dit is een adviesverdeling. De AI kan hier licht van afwijken afhankelijk van de inhoud.
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gray-900 rounded-2xl shadow-lg p-6 text-white">
                  ...
                  <p className="text-gray-400 text-sm mb-3">
                    {formData.sourceType === 'upload' 
                      ? "Kopieer deze prompt naar ChatGPT/Gemini en upload daarna direct je bestand."
                      : "Kopieer deze prompt (inclusief je tekst) naar ChatGPT/Gemini."}
                  </p>
                  <textarea 
                    readOnly
                    value={result.prompt}
