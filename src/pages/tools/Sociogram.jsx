...
const Sociogram=()=> {
  ...
  const [isStep2Expanded,setIsStep2Expanded]=true;
  ...
  const [highlightNodes,setHighlightNodes]=new Set();
  const [highlightLinks,setHighlightLinks]=new Set();
  ...
  form.setTitle('Sociogram: Hoe werken we samen in de klas?')
      .setDescription('Vul deze vragenlijst eerlijk in. Je antwoorden zijn alleen zichtbaar voor de leraar.\\n\\n' +
                     'Gebruik alleen namen van klasgenoten. Schrijf geen woorden zoals "niemand" of "iedereen".');
  
  form.addTextItem().setTitle('Wat is je voornaam?').setRequired(true);
...
function addNameQuestion(form, title) {
  var validation = FormApp.createTextValidation()
    ...
    .setHelpText('Gebruik alleen namen van klasgenoten. Schrijf geen "niemand" of "iedereen".')
    .build();

  form.addTextItem()
    .setTitle(title)
    .setHelpText('Meerdere namen scheiden met een komma.\\nSchrijf geen "niemand" of "iedereen", alleen namen.')
...
Pietje,Jantje,Anna,Klaasje,Pietje
Jantje,Anna,Pietje,Klaasje,Anna
Klaasje,Anna,Lisa,Anna,Pietje
Lisa,Tom,Jantje,Anna,Tom`;
  ...
  const generateGraph=()=> {
    const currentMapping = mapping.name ? mapping : {
      name: headers.find(h => ['hoe heet je','naam','leerling','student'].some(k => h.toLowerCase().includes(k))) || headers[0]
    };
    
    if (!currentMapping.name) return;
    
    const nodes=[];
    const links=[];
    const studentNamesSet=new Set();
    const forbiddenTerms=['de hele klas','heel de klas'];

    const normalizeAndSplit=(input)=> {
      if (!input) return [];
      return String(input).split(',').map(name=> {
        let clean=name.trim().replace(/\s+/g,' ');
        if (forbiddenTerms.includes(clean.toLowerCase())) return null;
        if (!clean) return null;
        return clean.charAt(0).toUpperCase() + clean.slice(1).toLowerCase();
      }).filter(name=> name !==null);
    };

    // HARD LOGIC: Alleen namen uit de respondenten-kolom (Kolom C) toevoegen als nodes
    parsedData.forEach(row=> {
      const respondents=normalizeAndSplit(row[currentMapping.name]);
      respondents.forEach(name=> studentNamesSet.add(name));
    });
    ...
    parsedData.forEach(row=> {
      ...
      const source=respondents[0];
      
      const createLinksForType=(colKey,linkType)=> {
        if (!currentMapping[colKey]) return;
        const targets=normalizeAndSplit(row[currentMapping[colKey]]);
        targets.forEach(target=> {
          // Edge case: Link alleen toevoegen als target ook in de whitelist (studentNamesSet) staat
          if (target !==source && studentNamesSet.has(target)) {
            links.push({source,target,type: linkType});
          }
        });
      };
      
      createLinksForType('socialPos','socialPos');
      createLinksForType('socialNeg','socialNeg');
      createLinksForType('workPos','workPos');
      createLinksForType('workNeg','workNeg');
    });
  ...
  const getLinkColor=(link)=> {
    ...
    const isHighlighted=highlightLinks && highlightLinks.size > 0 && highlightLinks.has(link);
    const opacity=highlightLinks && highlightLinks.size > 0 && !isHighlighted ? '0.05' : '0.6';
    switch (link.type) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {!isGenerated && (
          <div className="flex flex-wrap justify-center items-center gap-4 md:gap-12 mb-12">
            {[ {num: 1,label: 'Verzamel antwoorden',icon: FiSend},{num: 2,label: 'Controleer & exporteer',icon: FiDownload},{num: 3,label: 'Upload & visualiseer',icon: FiEye} ].map((step,i)=> (
          ...
          <motion.div initial={{opacity: 0,y: 20}} animate={{opacity: 1,y: 0}}>
            <div className="mb-10 p-6 bg-white rounded-2xl shadow-sm border border-blue-100">
              <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                <SafeIcon icon={FiInfo} className="text-blue-600" /> Zo gebruik je het Sociogram (kort uitgelegd)
              </h3>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                              <div>
                                ...
                                <div className="mt-10 pt-8 border-t border-gray-100">
                                  ...
                                  <img src={`${import.meta.env.BASE_URL}googlesheet.png`} alt="Uitleg Google Sheet opslaan" className="w-full h-auto rounded-xl my-4 border border-gray-200 shadow-sm" onError={(e)=> {e.target.onerror=null;e.target.style.display='none';}} />
                                  <div className="bg-green-50/50 rounded-xl p-5 border border-green-100">
                                    <h6 className="font-bold text-gray-900 mb-3 flex items-center gap-2"> <SafeIcon icon={FiDatabase} className="text-green-600" /> Stappenplan Google Sheet </h6>
                              ...
                              <div className="relative group">
                                <button onClick={copyScript} className={`absolute top-3 right-3 z-10 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 transition-all ${scriptCopied ? 'bg-green-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}> <SafeIcon icon={scriptCopied ? FiCheck : FiCopy} /> {scriptCopied ? 'Gekopieerd!' : 'Kopieer Script'} </button>
              ...
              <div className="bg-white rounded-2xl shadow-lg p-8 border border-gray-100">
                <div className="flex flex-wrap gap-4 mb-6">
                  <div className="flex space-x-2 p-1 bg-gray-100 rounded-lg">
                    <button onClick={()=> setActiveTab('sheet')} className={`px-4 py-2 rounded-md font-medium transition-colors text-sm ${activeTab==='sheet' ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-600'}`}> Google Sheet Link </button>
                    <button onClick={()=> setActiveTab('paste')} className={`px-4 py-2 rounded-md font-medium transition-colors text-sm ${activeTab==='paste' ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-600'}`}> Plakken </button>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="lg:col-span-8 bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col h-[700px]">
              ...
              <div className="flex-grow bg-slate-50 relative">
                <ForceGraph2D ref={graphRef} graphData={graphData} nodeLabel="name" nodeColor={node=> selectedNode?.id===node.id ? '#1e293b' : (highlightNodes?.has(node.id) ? '#2563eb' : '#94a3b8')} linkColor={getLinkColor} linkWidth={link=> (highlightLinks?.has(link) ? 3 : 1)} onNodeClick={node=> setSelectedNode(node)} nodeCanvasObject={(node,ctx,globalScale)=> {
