...
const { FiUpload, FiClipboard, FiSettings, FiActivity, FiDownload, FiSearch, FiEye, FiAlertTriangle, FiCheck, FiUsers, FiGrid, FiXCircle, FiInfo } = FiIcons;
...
const Sociogram = () => {
  ...
  const handlePasteProcess = () => {
    ...
    try {
      ...
      const rawLines = inputText.trim().split('\n');
      ...
      const candidates = [',', '\t', ';', '|']; // Volgorde van voorkeur
      let bestSeparator = ',';
      let maxCount = -1;
  ...
  const updateHighlight = useCallback(() => {
    ...
    if (activeId) {
      ...
      graphData.links.forEach(link => {
        ...
        if (link.source.id === activeId || link.target.id === activeId) {
          hLinks.add(link);
          ...
          >
            ...
            <div className="flex space-x-4 mb-6">
              <button 
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium transition-colors ${activeTab === 'paste' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'text-gray-600 hover:bg-gray-50'}`}
              >
              ...
              <button 
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium transition-colors ${activeTab === 'upload' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'text-gray-600 hover:bg-gray-50'}`}
              >
            ...
            {activeTab === 'paste' ? (
              <div className="mb-6">
                ...
                ></textarea>
                <p className="text-xs text-gray-500 mt-2">Ondersteunt data gescheiden door tabs, komma's, puntkomma's of pipes (|).</p>
              </div>
            ) : (
              ...
              <div className="mt-8 pt-8 border-t border-gray-100">
                ...
                {parseMessage && (
                  <div className={`mb-6 border rounded-lg p-4 flex items-start gap-3 ${parseMessage.type === 'error' ? 'bg-red-50 border-red-200' : 'bg-yellow-50 border-yellow-200'}`}>
                    <SafeIcon 
                        icon={parseMessage.type === 'error' ? FiXCircle : FiInfo} 
                        className={`text-xl mt-0.5 ${parseMessage.type === 'error' ? 'text-red-500' : 'text-yellow-600'}`} 
                    />
                    <div>
                ...
                <div className="mt-8 flex justify-end items-center gap-4">
                    ...
                    <button 
                        ...
                        className={`px-6 py-3 rounded-lg font-bold flex items-center space-x-2 transition-colors ${
                            isFormValid 
