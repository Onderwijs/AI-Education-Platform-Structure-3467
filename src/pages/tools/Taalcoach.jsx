...
- Leerlingprofiel: ${formData.profile}
- Vaardigheid: ${formData.skill}
- Thema / Context: ${formData.theme}
- Tijdsduur: ${formData.duration} minuten
- Type activiteit: ${formData.activityType}

Ontwerp een activiteit die:
- past bij het niveau en profiel van de leerling(en);
- duidelijk uitlegt wat de leerling moet doen;
- concrete instructies geeft voor de docent;
- een logische opbouw heeft (introductie, kern, afsluiting);
            ...
            <form onSubmit={generateActivity} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select name="targetGroup" value={formData.targetGroup} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 bg-white">
                    <option>Leerling (individueel)</option>
                    <option>Groep / klas</option>
                  </select>
                </div>
                <div>
                  ...
                  >
                    <option value="Basisschool (bovenbouw)">Basisschool (bovenbouw)</option>
                    ...
                    <option value="VMBO theoretisch">VMBO theoretisch</option>
              ...
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-3 top-2.5 text-gray-400"><SafeIcon icon={FiGlobe} /></span>
                  <select name="language" value={formData.language} onChange={handleChange} className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 bg-white">
                    <option>Nederlands</option>
                    <option>Engels</option>
                    <option>Duits</option>
                    <option>Frans</option>
                    <option>Spaans</option>
                    <option>Anders (specificeer)</option>
                  </select>
                </div>
                {formData.language === 'Anders (specificeer)' && (
                  <input type="text" name="customLanguage" placeholder="Welke taal?" value={formData.customLanguage} onChange={handleChange} className="mt-2 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500" />
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Vaardigheid</label>
                  <select name="skill" value={formData.skill} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 bg-white">
                    <option>Spreken</option>
                    <option>Luisteren</option>
                    <option>Lezen</option>
                    <option>Schrijven</option>
                    <option>Woordenschat</option>
                    <option>Grammatica</option>
                  </select>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="number" name="duration" min="5" max="120" required value={formData.duration} onChange={handleChange} className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500" />
                  </div>
                </div>
              </div>
              ...
              {/* Thema */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Thema / Context</label>
                <input type="text" name="theme" placeholder="Bijv. Vakantie, Solliciteren" required value={formData.theme} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500" />
              </div>

              {/* Profiel & Type */}
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Leerlingprofiel</label>
                  <div className="relative">
                     <span className="absolute left-3 top-2.5 text-gray-400"><SafeIcon icon={FiUser} /></span>
                     <select name="profile" value={formData.profile} onChange={handleChange} className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 bg-white">
                      <option>Algemeen</option>
                      <option>NT2 / Anderstalig</option>
                      <option>Taalzwak / Dyslexie</option>
                      <option>Hoogbegaafd</option>
                    </select>
                  </div>
                </div>
                 <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Activiteit</label>
                  <select name="activityType" value={formData.activityType} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 bg-white">
                    <option>Korte oefening</option>
                    <option>Volledige lesactiviteit</option>
                    <option>Huiswerkopdracht</option>
                  </select>
                </div>
              </div>
              ...
              {/* Bron Keuze */}
              <div className="pt-2">
                <label className="block text-sm font-medium text-gray-700 mb-2">Inhoud / Bron</label>
                <div className="space-y-2">
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="radio" name="sourceType" value="ai" checked={formData.sourceType === 'ai'} onChange={handleChange} className="text-cyan-600 focus:ring-cyan-500" />
                    <span className="text-sm text-gray-700">AI bedenkt passende tekst/woorden</span>
                  </label>
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="radio" name="sourceType" value="user" checked={formData.sourceType === 'user'} onChange={handleChange} className="text-cyan-600 focus:ring-cyan-500" />
                    <span className="text-sm text-gray-700">Ik plak zelf een tekst of woordenlijst</span>
                  </label>
                </div>

                {formData.sourceType === 'user' && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-2">
                    <textarea 
                      name="sourceText" 
                      rows="3" 
                      placeholder="Plak hier je tekst of woordenlijst..." 
              ...
              <button 
                type="submit" 
                className="w-full bg-cyan-600 text-white py-3 px-4 rounded-lg font-semibold hover:bg-cyan-700 transition-colors shadow-md flex items-center justify-center space-x-2 mt-4"
              >
                <SafeIcon icon={FiEdit3} />
                <span>Genereer Taalactiviteit & Prompt</span>
              </button>
            </form>
          </motion.div>
          ...
          <div className="lg:col-span-7 space-y-6">
            {!result ? (
              <div className="bg-white border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center h-full flex flex-col items-center justify-center text-gray-400 min-h-[400px]">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">Nog geen activiteit gegenereerd</h3>
              ...
              >
                ...
                <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                  <div className="bg-cyan-50 p-4 border-b border-cyan-100">
                    <h3 className="font-bold text-cyan-900 flex items-center gap-2 text-lg">
                      <SafeIcon icon={FiList} /> {result.proposal.title}
                    </h3>
                  </div>
                  <div className="p-6">
                    ...
                    <div className="mb-4">
                      <h4 className="text-sm font-bold text-gray-900 uppercase tracking-wide mb-2">Activiteit Opbouw</h4>
                      <ul className="space-y-2">
                        {result.proposal.steps.map((step, idx) => (
                          <li key={idx} className="flex items-start gap-2 text-sm text-gray-700">
                            <span className="bg-cyan-100 text-cyan-800 font-bold w-5 h-5 rounded-full flex items-center justify-center text-xs flex-shrink-0 mt-0.5">{idx + 1}</span>
                            <span>{step}</span>
                          </li>
                        ))}
                      </ul>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-4 border-t border-gray-100">
                      <div>
                        <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Differentiatie ({formData.profile})</h4>
                        <ul className="list-disc list-inside text-sm text-gray-600 space-y-1">
                          {result.proposal.differentiation.map((tip, idx) => (
                            <li key={idx}>{tip}</li>
                          ))}
                        </ul>
                      </div>
                      <div>
                        <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Benodigdheden</h4>
                         <ul className="list-disc list-inside text-sm text-gray-600 space-y-1">
                          {result.proposal.materials.map((mat, idx) => (
                            <li key={idx}>{mat}</li>
                          ))}
                        </ul>
                      </div>
                ...
                <div className="bg-gray-900 rounded-2xl shadow-lg p-6 text-white">
                  ...
                  <p className="text-gray-400 text-sm mb-3">
                    Kopieer deze prompt naar ChatGPT of Gemini om de volledige les/materialen te genereren.
                  </p>
                  <textarea 
                    readOnly
                    value={result.prompt}
                    className="w-full h-64 bg-gray-800 text-gray-300 text-sm font-mono p-4 rounded-xl border border-gray-700 focus:outline-none focus:ring-2 focus:ring-cyan-500 resize-none"
                  />
                </div>
              </motion.div>
            )}
