...
const RubricMaker = () => {
  ...
  const generateRubric = (e) => {
    ...
    const promptText = `Je bent een ervaren docent. Ontwerp een beoordelingsrubriek voor de volgende opdracht.

Titel van de opdracht: ${formData.title}
Vak / context: ${formData.subject}
Niveau / klas: ${formData.level}
Beschrijving van de opdracht: ${formData.description}

Maak een rubriek met ${formData.criteriaCount} criteria.
Voor elk criterium:
- Geef een naam
- Voeg een korte beschrijving toe
- Beschrijf 4 prestatieniveaus (Onvoldoende, Voldoende, Goed, Uitstekend)
- Maak de beschrijvingen concreet en observeerbaar.

Lever de rubriek aan in een duidelijk gestructureerde tabel.`;
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    name="title" 
                    required
                    placeholder="Bijv. Essay Gouden Eeuw" 
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <textarea 
                    ...
                    required
                    rows="4"
                    placeholder="Wat moeten de leerlingen doen? Wat zijn de belangrijkste eisen?" 
              ...
              >
                ...
                <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                  <div className="bg-pink-50 p-6 border-b border-pink-100">
                    ...
                    <div className="flex flex-wrap gap-2 text-sm text-pink-700 font-medium mb-3">
                      <span className="bg-pink-100 px-2 py-1 rounded">{result.rubric.meta}</span>
                    </div>
                    <p className="text-gray-600 text-sm italic">"{result.rubric.description}"</p>
                  </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          <th scope="col" className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-1/5">
                            Criterium
                          </th>
                          {result.rubric.headers.map((header, i) => (
                            <th key={i} scope="col" className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-1/5">
                              {header}
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {result.rubric.rows.map((row, rowIndex) => (
                          <tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                            <td className="px-6 py-4 text-sm font-bold text-gray-900 align-top">
                              {row.name}
                            </td>
                            {row.cells.map((cell, cellIndex) => (
                              <td key={cellIndex} className="px-6 py-4 text-sm text-gray-600 align-top">
                                {cell}
                              </td>
                            ))}
                          </tr>
                  ...
                  <div className="p-4 bg-gray-50 text-xs text-gray-500 text-center border-t border-gray-100">
                    * Dit is een conceptrubriek. Pas de criteria aan op basis van jouw specifieke leerdoelen.
                  </div>
                </div>
                ...
                <div className="bg-gray-900 rounded-2xl shadow-lg p-6 text-white">
                  ...
                  <p className="text-gray-400 text-sm mb-3">
                    Wil je een meer gedetailleerde rubriek? Kopieer deze prompt naar ChatGPT of Gemini.
                  </p>
                  <textarea 
                    readOnly
                    value={result.prompt}
                    className="w-full h-48 bg-gray-800 text-gray-300 text-sm font-mono p-4 rounded-xl border border-gray-700 focus:outline-none focus:ring-2 focus:ring-pink-500 resize-none"
                  />
                </div>
              </motion.div>
            )}
