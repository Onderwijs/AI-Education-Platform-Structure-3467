  ...
  const trainings=[ 
    {
      ...
      topics: [ 
        ...
        "AI inzetten ter ondersteuning van lesontwerp en onderwijspraktijk",
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"> 
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center"> 
            <motion.div initial={{opacity: 0,x: -50}} animate={{opacity: 1,x: 0}} > 
              ...
              <p className="text-xl text-blue-100 mb-8"> Praktische workshops en cursussen om AI effectief te integreren in jouw onderwijspraktijk. Voor alle niveaus en onderwijslagen. </p> 
      ...
      <section className="py-16 bg-white"> 
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"> 
          <motion.div initial={{opacity: 0,y: 50}} whileInView={{opacity: 1,y: 0}} viewport={{once: true}} className="text-center mb-12" > 
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4"> Waarom kiezen voor trainingen? </h2> 
            <p className="text-xl text-gray-600 max-w-3xl mx-auto"> Trainingen zijn ontwikkeld door ervaren docenten en AI-experts </p> 
          </motion.div> 
      ...
      <section id="trainingen" className="py-20 bg-gray-50"> 
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"> 
          <motion.div initial={{opacity: 0,y: 50}} whileInView={{opacity: 1,y: 0}} viewport={{once: true}} className="text-center mb-16" > 
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4"> Trainingen </h2> 
            <p className="text-xl text-gray-600"> Van beginnende tot ervaren AI-gebruikers in het onderwijs </p> 
          </motion.div> 
                    ...
                    <ul className="space-y-1"> 
                      {training.topics.map((topic,topicIndex)=> ( 
                        <li key={topicIndex} className="text-sm text-gray-600 flex items-start space-x-2" > 
                          <div className="w-1.5 h-1.5 bg-blue-500 rounded-full mt-1.5"></div> <span>{topic}</span> </li> 
                      ))} 
                    </ul> 
                  </div> 
                  <div className="mb-4"> 
                    ...
                    <ul className="space-y-1"> 
                      {training.nextDates.map((date,dateIndex)=> ( 
                        <li key={dateIndex} className="text-sm text-gray-600" > {date} </li> 
                      ))} 
                    </ul> 
                  </div> 
                  <div className="flex items-center justify-between pt-4 border-t"> 
                    ...
                    {training.isExternal ? ( 
                      <button onClick={()=> handleExternalLink(training.link)} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors flex items-center space-x-2" > 
                        <span>Inschrijven</span> <SafeIcon icon={FiExternalLink} className="text-sm" /> </button> 
                    ) : ( 
                      <button className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors"> Inschrijven </button> 
                    )} 
                  </div> 
                  {training.priceNote && (
          ...
          <motion.div initial={{opacity: 0,y: 50}} whileInView={{opacity: 1,y: 0}} viewport={{once: true}} className="mt-16 text-center" > 
            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-8 max-w-4xl mx-auto"> 
              ...
              <Link to="/over-ons#contact" className="bg-blue-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-blue-700 transition-colors inline-flex items-center space-x-2" > 
                <SafeIcon icon={FiMail} /> <span>Stuur een bericht</span> </Link> 
            </div> 
          </motion.div> 
        </div> 
      </section> 
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center"> 
          <motion.div initial={{opacity: 0,y: 50}} whileInView={{opacity: 1,y: 0}} viewport={{once: true}} > 
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center"> 
              <a href="#trainingen" className="bg-white text-blue-600 px-8 py-4 rounded-lg font-semibold hover:bg-gray-100 transition-colors" > Bekijk trainingen </a> 
            </div> 
          </motion.div> 
        </div> 
      </section> 
